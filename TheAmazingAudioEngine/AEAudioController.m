...
+ (NSError *)audioControllerErrorWithMessage:(NSString *)message OSStatus:(OSStatus)status {
    ...
    return [NSError errorWithDomain:NSOSStatusErrorDomain
                               code:status
                           userInfo:@{NSLocalizedDescriptionKey : [NSString stringWithFormat:@"%@ (error %d/%4.4s)",
                                                                                             message,
                                                                                             (int) status,
                                                                                             (char *) &fourCC]}];
}
...
struct fillComplexBufferInputProc_t {
...
- (id)initWithAudioDescription:(AudioStreamBasicDescription)audioDescription
                  inputEnabled:(BOOL)enableInput
            useVoiceProcessing:(BOOL)useVoiceProcessing
                 outputEnabled:(BOOL)enableOutput {
...
- (BOOL)updateWithAudioDescription:(AudioStreamBasicDescription)audioDescription
                      inputEnabled:(BOOL)enableInput
                useVoiceProcessing:(BOOL)useVoiceProcessing
                     outputEnabled:(BOOL)enableOutput {
...
- (BOOL)start:(NSError **)error recoveringFromErrors:(BOOL)recoverFromErrors {
    ...
    if (!checkResult(status, "AUGraphStart")) {
        if (!recoverFromErrors || ![self attemptRecoveryFromSystemError:error]) {
            NSError *startError = [NSError audioControllerErrorWithMessage:@"Couldn't start audio engine"
                                                                  OSStatus:status];
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:AEAudioControllerErrorOccurredNotification
                                                                object:self
                                                              userInfo:@{AEAudioControllerErrorKey : startError}];
            return NO;
        }
    }
    if (!self.running) {
...
- (void)addChannels:(NSArray *)channels toChannelGroup:(AEChannelGroupRef)group {
    ...
    for (id <AEAudioPlayable> channel in channels) {
        ...
        for (NSString *property in @[@"volume", @"pan", @"channelIsPlaying", @"channelIsMuted", @"audioDescription"]) {
            [(NSObject *) channel addObserver:self
                                   forKeyPath:property
                                      options:0
                                      context:kChannelPropertyChanged];
        }
    ...
    [self configureChannelsInRange:NSMakeRange(group->channelCount - channels.count, channels.count)
                          forGroup:group];
...
- (void)removeChannels:(NSArray *)channels {
    ...
    for (id <AEAudioPlayable> channel in channels) {
        AEChannelGroupRef group = [self searchForGroupContainingChannelMatchingPtr:channel.renderCallback
                                                                          userInfo:(__bridge void *) channel
                                                                             index:NULL];
...
- (void)removeChannelGroup:(AEChannelGroupRef)group {
    ...
    // Find group's parent
    int index;
    AEChannelGroupRef parentGroup = (group == _topGroup ? NULL : [self searchForGroupContainingChannelMatchingPtr:group
                                                                                                         userInfo:NULL
                                                                                                            index:&index]);
    ...
    if (parentGroup) {
        ...
        [self configureChannelsInRange:NSMakeRange(0, parentGroup->channelCount)
                              forGroup:parentGroup];
        checkResult([self updateGraph], "Update graph");
    }
...
- (void)setVolume:(float)volume forChannelGroup:(AEChannelGroupRef)group {
    int index;
    AEChannelGroupRef parentGroup = [self searchForGroupContainingChannelMatchingPtr:group
                                                                            userInfo:NULL
                                                                               index:&index];
...
- (void)addFilter:(id <AEAudioFilter>)filter {
    if ([self addCallback:filter.filterCallback
                 userInfo:(__bridge void *) filter
                    flags:kFilterFlag
          forChannelGroup:_topGroup]) {
...
- (void)addInputFilter:(id <AEAudioFilter>)filter forChannels:(NSArray *)channels {
    ...
    if ([self addCallback:callback
                 userInfo:(__bridge void *) filter
                    flags:kFilterFlag
         forInputChannels:channels]) {
        CFBridgingRetain(filter);
    }
}

- (void)removeFilter:(id <AEAudioFilter>)filter {
    if ([self removeCallback:filter.filterCallback
                    userInfo:(__bridge void *) filter
            fromChannelGroup:_topGroup]) {
...
- (NSArray *)inputFilters {
    ...
    for (int i = 0; i < _inputCallbackCount; i++) {
        [result addObjectsFromArray:[self associatedObjectsFromTable:&_inputCallbacks[i].callbacks
                                                        matchingFlag:kFilterFlag]];
    }
    return result;
}
...
- (void)addOutputReceiver:(id <AEAudioReceiver>)receiver {
    if ([self addCallback:receiver.receiverCallback
                 userInfo:(__bridge void *) receiver
                    flags:kReceiverFlag
          forChannelGroup:_topGroup]) {
...
- (void)removeOutputReceiver:(id <AEAudioReceiver>)receiver {
    if ([self removeCallback:receiver.receiverCallback
                    userInfo:(__bridge void *) receiver
            fromChannelGroup:_topGroup]) {
...
- (void)outputAveragePowerLevels:(Float32 *)averagePowers
    ...
    return [self averagePowerLevels:averagePowers
                     peakHoldLevels:peakLevels
                           forGroup:_topGroup
                       channelCount:count];
}
...
- (void)averagePowerLevel:(Float32 *)averagePower
    ...
    if (!group->level_monitor_data.monitoringEnabled) {
        if (![NSThread isMainThread]) {
            ...
            int index = 0;
            if (group != _topGroup) {
                parentGroup = [self searchForGroupContainingChannelMatchingPtr:group
                                                                      userInfo:NULL
                                                                         index:&index];
...
- (void)setInputGain:(float)inputGain {
    NSError *error = NULL;
    if (![((AVAudioSession *) [AVAudioSession sharedInstance]) setInputGain:inputGain
                                                                      error:&error]) {
...
- (void)setAudiobusSenderPort:(ABSenderPort *)audiobusSenderPort
            forChannelElement:(AEChannelRef)channelElement {
    ...
    if (audiobusSenderPort == nil) {
        ...
        if (channelElement->type == kChannelTypeGroup) {
            ...
            if (channelElement != _topChannel) {
                parentGroup = [self searchForGroupContainingChannelMatchingPtr:channelElement->ptr
                                                                      userInfo:NULL
                                                                         index:&index];
                NSAssert(parentGroup != NULL, @"Channel group not found");
            }
...
- (void)applicationWillEnterForeground:(NSNotification *)notification {
    ...
    if (_interrupted) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:AEAudioControllerSessionInterruptionEndedNotification
                                                            object:self];
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([notification.userInfo[AVAudioSessionInterruptionTypeKey] intValue] == AVAudioSessionInterruptionTypeEnded) {
            ...
            if ([[UIApplication sharedApplication] applicationState] != UIApplicationStateBackground || _started) {
                ...
                NSError *error = nil;
                if (![((AVAudioSession *) [AVAudioSession sharedInstance]) setActive:YES
                                                                               error:&error]) {
                    NSLog(@"TAAE: Couldn't activate audio session: %@", error);
                }
            }
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:AEAudioControllerSessionInterruptionBeganNotification
                                                                object:self];
            processPendingMessagesOnRealtimeThread(self);
        }
    });
...
- (void)audioRouteChangeNotification:(NSNotification *)notification {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([currentRoute.outputs filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"portType = %@",
                                                                                               AVAudioSessionPortBuiltInSpeaker]].count > 0) {
        ...
        if ([currentRoute.inputs filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"portType = %@",
                                                                                              AVAudioSessionPortBuiltInMic]].count > 0) {
        ...
        if (_recordingThroughDeviceMicrophone != recordingThroughMic) {
            ...
            if (_useMeasurementMode && _avoidMeasurementModeForBuiltInMic) {
                ...
                if (![audioSession setMode:_useMeasurementMode && !_recordingThroughDeviceMicrophone ? AVAudioSessionModeMeasurement : AVAudioSessionModeDefault
                    ...
                    if (![audioSession setPreferredIOBufferDuration:_preferredBufferDuration
                                                              error:&error]) {
                        NSLog(@"TAAE: Couldn't set preferred IO buffer duration: %@", error);
                    }
                }
...
- (BOOL)setup {
    ...
    if (!checkResult(result, "AUGraphNodeInfo")) {
        return NO;
    }
#ifdef DEBUG
    ...
    if (!checkResult(result, "AUGraphInitialize")) {
        self.lastError = [NSError audioControllerErrorWithMessage:@"Couldn't create audio graph"
                                                         OSStatus:result];
        [[NSNotificationCenter defaultCenter] postNotificationName:AEAudioControllerErrorOccurredNotification
                                                            object:self
                                                          userInfo:@{AEAudioControllerErrorKey : _lastError}];
        _hasSystemError = YES;
        return NO;
    }
    ...
    if ((_audiobusReceiverPort && ABReceiverPortIsConnected(_audiobusReceiverPort)) || (_audiobusFilterPort && ABFilterPortIsConnected(_audiobusFilterPort))) {
        ...
        if (inputAvailable) {
            ...
            if (hasChannelCount) {
                ...
                if (!_lastError) {
                    self.lastError = [NSError audioControllerErrorWithMessage:@"Audio system error while determining input channel count"
                                                                     OSStatus:(OSStatus) channels];
                    [[NSNotificationCenter defaultCenter] postNotificationName:AEAudioControllerErrorOccurredNotification
                                                                        object:self
                                                                      userInfo:@{AEAudioControllerErrorKey : _lastError}];
                }
        ...
        if (channel->type == kChannelTypeChannel) {
            ...
            if (channel->audiobusFloatConverter) {
                ...
                if (memcmp(&converterFormat, &channel->audioDescription, sizeof(channel->audioDescription)) != 0) {
                    ...
                    [self performAsynchronousMessageExchangeWithBlock:^{
                            channel->audiobusFloatConverter = newFloatConverter;
                        }
                                                        responseBlock:^{
                                                            CFBridgingRelease(oldFloatConverter);
                                                        }];
            ...
            if (subgroup->level_monitor_data.monitoringEnabled) {
                ...
                if (memcmp(&converterFormat, &channel->audioDescription, sizeof(channel->audioDescription)) != 0) {
                    ...
                    [self performAsynchronousMessageExchangeWithBlock:^{
                            subgroup->level_monitor_data.floatConverter = newFloatConverter;
                        }
                                                        responseBlock:^{
                                                            CFBridgingRelease(oldFloatConverter);
                                                        }];
...
- (AEChannelGroupRef)searchForGroupContainingChannelMatchingPtr:(void *)ptr
                                                       userInfo:(void *)userInfo
                                                    withinGroup:(AEChannelGroupRef)group
                                                          index:(int *)index {
    ...
    for (int i = 0; i < group->channelCount; i++) {
        ...
        if (channel->type == kChannelTypeGroup) {
            AEChannelGroupRef match = [self searchForGroupContainingChannelMatchingPtr:ptr
                                                                              userInfo:userInfo
                                                                           withinGroup:channel->ptr
                                                                                 index:index];
...
- (AEChannelGroupRef)searchForGroupContainingChannelMatchingPtr:(void *)ptr
                                                       userInfo:(void *)userInfo
                                                          index:(int *)index {
    return [self searchForGroupContainingChannelMatchingPtr:ptr
                                                   userInfo:userInfo
                                                withinGroup:_topGroup
                                                      index:(int *) index];
}
...
- (BOOL)attemptRecoveryFromSystemError:(NSError **)error {
    ...
    while (retries > 0) {
        ...
        if ([self setup]) {
            [[NSNotificationCenter defaultCenter] postNotificationName:AEAudioControllerDidRecreateGraphNotification
                                                                object:self];
...
- (BOOL)addCallback:(void *)callback
           userInfo:(void *)userInfo
              flags:(uint8_t)flags
         forChannel:(id <AEAudioPlayable>)channelObj {
    int index = 0;
    AEChannelGroupRef parentGroup = [self searchForGroupContainingChannelMatchingPtr:channelObj.renderCallback
                                                                            userInfo:(__bridge void *) channelObj
                                                                               index:&index];
    NSAssert(parentGroup != NULL, @"Channel not found");
...
- (BOOL)addCallback:(void *)callback
           userInfo:(void *)userInfo
              flags:(uint8_t)flags
   forInputChannels:(NSArray *)channels {
...
- (BOOL)removeCallback:(void *)callback
              userInfo:(void *)userInfo
           fromChannel:(id <AEAudioPlayable>)channelObj {
    int index = 0;
    AEChannelGroupRef parentGroup = [self searchForGroupContainingChannelMatchingPtr:channelObj.renderCallback
                                                                            userInfo:(__bridge void *) channelObj
                                                                               index:&index];
...
- (NSString *)stringFromRouteDescription:(AVAudioSessionRouteDescription *)routeDescription {
    ...
    return [NSString stringWithFormat:@"%@%@%@",
                                      inputsString,
                                      inputsString.length > 0 && outputsString.length > 0 ? @" and " : @"",
                                      outputsString];
}
        ...
        while (![self isCancelled]) {
            @autoreleasepool {
                if (AEAudioControllerHasPendingMainThreadMessages(_audioController)) {
                    [_audioController performSelectorOnMainThread:@selector(pollForMessageResponses)
                                                       withObject:nil
                                                    waitUntilDone:NO];
                }
